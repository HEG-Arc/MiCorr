...
class GraphGenerationUtil {
    ...
    drawStrata(strata, divID, draw) {
        ...
        let label = nestedStrata.plain(strata.getLabel()).attr({x:20, y:height/2, "text-anchor":"auto","font-size":15}).fill((strataColor!=="black") ? "black" : "white");
        ...
        label.before(nestedStrata.rect(labelBbox.width * wRectRatio, labelBbox.height * hRectRatio)
            .attr({x:labelBbox.x-labelBbox.width*((wRectRatio-1)/2), y:labelBbox.y-labelBbox.height*((hRectRatio-1)/2), rx:5,ry:5}).
            fill({color:"#b0b0b0",opacity:"0.6"}));
