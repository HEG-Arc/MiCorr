...
angular.module('micorrApp').directive('strata', function($compile, StrataData){
    return {
        ...
        link : function(scope, element, attrs) {
            ...
            if (strat.getNatureFamilyUid() == "cmCharacteristic"){
                cmStrat = scope.rstratas[parseInt(index)+1];
                cpStrat = scope.rstratas[parseInt(index)-1];
                drawStrat(cpStrat);
                if (cmStrat.findDependency('colourFamily')) {
                    if (cmStrat.getColourFamily() != "" && typeof cmStrat.getColourFamily() != "undefined")
                        colour = returnFormattedColor(cmStrat.getColourFamily());
                }
                else
                    colour = "white";
                ...
                bx = 3;
                by = 0;
                cx = 3+(scope.ratio.ratio*10);
                cy = ay;

                paper.path("M"+String(ax)+","+String(ay)+","+String(bx)+","+String(by)+","+String(cx)+","+String(cy)+"z").attr({
                    fill: colour,
                    "stroke-width": 0
                });

                ax = (width/6)-(scope.ratio.ratio*10);
                ay = height;
                bx = width/6;
                by = 0;
                cx = (width/6)+(scope.ratio.ratio*10);
                cy = ay;
                ...
                ax = (width/3)-(scope.ratio.ratio*10);
                ay = height;
                bx = width/3;
                by = 0;
                cx = (width/3)+(scope.ratio.ratio*10);
                cy = ay;
                ...
                ax = (width/2)-(scope.ratio.ratio*10);
                ay = height;
                bx = width/2;
                by = 0;
                cx = (width/2)+(scope.ratio.ratio*10);
                cy = ay;
                ...
                ax = (2*width/3)-(scope.ratio.ratio*10);
                ay = height;
                bx = 2*width/3;
                by = 0;
                cx = (2*width/3)+(scope.ratio.ratio*10);
                cy = ay;
                ...
                ax = (5*width/6)-(scope.ratio.ratio*10);
                ay = height;
                bx = 5*width/6;
                by = 0;
                cx = (5*width/6)+(scope.ratio.ratio*10);
                cy = ay;
                ...
                paper.path("M"+String(ax)+","+String(ay)+","+String(bx)+","+String(by)+","+String(cx)+","+String(cy)+"z").attr({
                    fill: "url(../static/micorr/images/c/hexagonal.png)",
                    "stroke-width": 0
                });
                ...
                // Porosity
                if (cmStrat.findDependency('porosityFamily')){
                    var img = 'porosity';
                    if (cmStrat.getPorosityFamily() == 'slightlyPorousCharacteristic')
                        poisson.push({'min' : 20, 'max' : 90, 'img' : img, 'imgw' : 5, 'imgh' : 5});
                        //pds.createPointsPerso(90, 90, img);
                    if (cmStrat.getPorosityFamily() == 'porousCharacteristic')
                        poisson.push({'min' : 20, 'max' : 40, 'img' : img, 'imgw' : 5, 'imgh' : 5});
                        //pds.createPointsPerso(40, 40, img);
                    if (cmStrat.getPorosityFamily() == 'highlyPorousCharacteristic')
                        poisson.push({'min' : 20, 'max' : 20, 'img' : img, 'imgw' : 5, 'imgh' : 5});
                        //pds.createPointsPerso(20, 20, img);
                }
            }
